...
// Debouncing is quite very useful & powerful in developing real-world web-apps which can improve the performance of web-apps

// Debouncing is a technique which optimizes the application & browser performances

// delay is between two key-press events


// Debounce function limits the execution of a function call and waits for certain amount of time before running it again
...
function getHotelCard(hotel) {
    ...
    <div>
      ...
      <span>${hotel.eta} mins</span>
      <button>View Menu</button>
      <br/>
      <button id="fav${hotel.id}" onclick="setFav(${hotel.id})" class="${ButtonClass}">
      ${ButtonText}
      </button>
    </div>
    </li>
...
function getFilteredResult() {
    ...
    filterItem = filterDropdown.value
    if (filterItem) {
        filteredHotels = filteredHotels.filter((item) => {
